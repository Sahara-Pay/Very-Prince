...
/**
 * XDR Viewer page — demonstrates the streaming XDR parser.
 *
 * When no `xdr` search param is present, renders the input form (client component).
 * When `?xdr=<base64>` is present, renders the `XdrStreamServer` server component
 * which decodes the XDR on the Node.js server and streams operations chunk-by-chunk.
 */
export default async function XdrViewerPage({
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 p-6">
      ...
      {/* Streamed output — only rendered when xdr param is present */}
      {xdr ? (
